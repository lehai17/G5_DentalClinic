...
public interface SlotRepository extends JpaRepository<Slot, Long> {
        ...
        @Query("SELECT COALESCE(SUM(s.capacity), 0) FROM Slot s WHERE s.slotTime >= :start AND s.slotTime < :end AND s.active = true")
        long sumCapacityActiveInRange(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
        ...
        @Query("SELECT s FROM Slot s WHERE s.slotTime >= :fromTime AND s.slotTime < :toTime ORDER BY s.slotTime ASC")
        ...
        /**
         * Get available slots within a time range for TODAY ONLY (no > currentTime
         * filter).
         * This returns slots from current time onwards without filtering based on exact
         * currentTime.
         * Used when selectedDate == LocalDate.now() to ensure current slot and future
         * slots are included.
         */
        @Query("SELECT s FROM Slot s WHERE s.slotTime >= :fromTime AND s.slotTime < :toTime AND s.active = true AND s.bookedCount < s.capacity ORDER BY s.slotTime ASC")
         ...
         * This returns slots from current time onwards without filtering based on exact
         * currentTime.
         * Used when selectedDate == LocalDate.now() to show all slots with time >=
         * current slot time.
         */
        @Query("SELECT s FROM Slot s WHERE s.slotTime >= :fromTime AND s.slotTime < :toTime AND s.active = true ORDER BY s.slotTime ASC")
        ...
        @Modifying
        @Transactional
        @Query("UPDATE Slot s SET s.active = false, s.lockReason = :reason " +
        ...
        @Modifying
        @Transactional
        @Query("UPDATE Slot s SET s.active = true, s.lockReason = null " +
                        "WHERE s.slotTime >= :start AND s.slotTime <= :end")
        void enableSlotsInPeriod(@Param("start") LocalDateTime start,
                        @Param("end") LocalDateTime end);
